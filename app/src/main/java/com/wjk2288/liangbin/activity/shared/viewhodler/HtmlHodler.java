...
public class HtmlHodler extends RecyclerView.ViewHolder {
    ...
    public HtmlHodler(View itemView) {
        super(itemView);
    ...
    public void setData(int position, List<RecommedBean.ListBean> datas, Context context) {
        ...
        String text = datas.get(position).getHtml().getDesc();
        ...
        String htmlUrl = datas.get(position).getHtml().getThumbnail_small().get(0);

        Glide.with(context).load(htmlUrl)
                .into(ivIcon);
